...
public static class ReplayTests
{
    public static async Task RunAsync(List<string> failures)
    {
        ...
        store1.Profiles[userId] = new MemberProfile { UserId = userId, WalletBalance = 1000m, Username = "replay" };
        ...
        store2.Profiles[userId] = new MemberProfile { UserId = userId, WalletBalance = 1000m, Username = "replay" };

        // 3. Execute Parallel Action: Deal
        var deal1 = await svc1.DealAsync(userId, new DealRequest(machineId, 10m), CancellationToken.None);
        var deal2 = await svc2.DealAsync(userId, new DealRequest(machineId, 10m), CancellationToken.None);
